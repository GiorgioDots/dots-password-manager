...
import type { AuthTokenResponse, RefreshTokenRequest } from '#/lib/auth/contracts'
...
export const Route = createFileRoute('/api/auth/refresh-token')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const body = (await request.json()) as Partial<RefreshTokenRequest>
        ...
        if (!existingToken || existingToken.expiresAt <= new Date()) {
          return errorResponse('Invalid or expired refresh token.', 422)
        }
